...
step3: Steps to create Gcloud Project
      ...
      d. Create a project named "flaskhelloworldapp" and navigate to your newly generated project dashboard.
...
Step 4: To deploy application to gcloud platform:
      ...
      NOTE: In case the above command leads to an error during deployment, try running `docker buildx build --platform linux/amd64 --load -t gcr.io/<YOUR_PROJECT_ID>/<YOUR_PROJECT_NAME> .`
      ...
      e. Now we can see newest created image has tag as <none> and its image id.
         type ‘docker tag <your newest image id> gcr.io/<your_project_id>/<project_name>' and hit enter and then type docker images. 
